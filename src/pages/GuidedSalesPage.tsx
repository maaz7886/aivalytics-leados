...
export default function GuidedSalesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'queue' | 'tree'>('tree');
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 text-white rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden border border-purple-800/50">
        ...
        <div className="relative z-10 space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-purple-500/20 border border-purple-400/30 rounded-full text-purple-300 text-xs font-extrabold tracking-wide uppercase">
            <span>🧠 24-Phase Consultative Sales System</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white">
            Aivalytics AI LeadOS — Guided Sales Decision Engine
          </h1>
          <p className="text-purple-200 text-sm max-w-3xl leading-relaxed">
            ...
            Provides an interactive decision tree flowchart, live 1–3 sentence spoken teleprompter, Hormozi value equation drivers, 11-objection resolution matrix, and 1-click Razorpay payment link dispatch.
          </p>
        </div>
      </div>

      {/* Primary Tab Navigation Selector */}
      <div className="flex border-b border-gray-200 dark:border-gray-700 space-x-4 bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-xs">
        <button
        ...
        >
          <span>🌳</span> Interactive 24-Phase Decision Tree Map
        </button>
        ...
        >
          <span>📞</span> Live Prospect Call Queue & Teleprompter
        </button>
      </div>

      {/* TAB 1: INTERACTIVE 24-PHASE DECISION TREE MAP */}
      {activeTab === 'tree' && (
      ...
      {/* TAB 2: PROSPECT CALL QUEUE & ACTIVE CALL HUD */}
      {activeTab === 'queue' && (
