...
export default function Dashboard() {
  ...
  const contactedCount = filteredLeads.filter(
    (l) => l.numberOfCalls > 0 || (l.lastContacted && l.lastContacted !== 'Not Contacted') || (l.crmStage !== 'Lead' && l.crmStage !== 'New Lead')
  ).length;

  const qualifiedCount = filteredLeads.filter(
    (l) => l.crmStage === 'Qualified' || l.crmStage === 'Details sent' || l.crmStage === 'Seat Reserved' || l.crmStage === 'Enrolled'
  ).length;
  ...
  const enrollmentsCount = filteredLeads.filter(
    (l) => l.crmStage === 'Enrolled' || l.paymentStatus === 'Paid' || l.enrollmentStatus === 'Enrolled'
  ).length;
  ...
  const funnelData = [
    ...
    { name: 'Seat Reserved', value: seatReservedStageCount || 1, fill: '#55BD71' },
    { name: 'Enrolled', value: enrolledStageCount || 1, fill: '#80D497' }
  ];

  // Dynamic Negative / Nurture Breakdown
  const noResponseCount = filteredLeads.filter((l) => l.crmStage === "Didn't attempt the call" || l.lastContacted === 'Not Contacted').length;
  const nurtureCount = filteredLeads.filter((l) => l.crmStage === 'Details sent' || l.crmStage === 'Interested').length;
  const notInterestedCount = filteredLeads.filter((l) => l.crmStage === 'Not interested').length;
  const unqualifiedCount = filteredLeads.filter((l) => l.crmStage === 'Not qualified' || l.crmStage === 'Invalid number').length;
