...
export function mapLeadToDb(lead: Partial<Lead>): any {
  return {
    ...
    date_captured: lead.dateCaptured || lead.dateOfFillingForm || new Date().toISOString(),
    ...
    years_of_experience: Number(lead.yearsOfExperience || (lead.experience ? parseInt(lead.experience) : 0) || 0),
    ...
    current_ai_usage_level: lead.aiUsage || lead.currentAiUsageLevel || 'Beginner',
    primary_goal: lead.goal || lead.primaryGoal || '',
    desired_role: lead.desiredRole || '',
    expected_timeline: lead.expectedTimeline || '',
    main_challenge: lead.blocker || lead.mainChallenge || '',
    why_now: lead.whyNow || '',
...
export function mapDbToLead(d: any): Lead {
  return {
    ...
    dateOfFillingForm: d.date_captured ? String(d.date_captured).split('T')[0] : '',
    createdTime: d.date_captured ? String(d.date_captured).replace('T', ' ').substring(0, 16) : (d.created_at ? String(d.created_at).replace('T', ' ').substring(0, 16) : ''),
